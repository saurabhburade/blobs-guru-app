...
import { CELESTIA_PRICE_DAY_DATAS_QUERY } from "@/lib/apollo/queriesCelestia";
...
export default function PriceDayChart({ duration }: { duration: number }) {
  const { data, loading } = useQuery(CELESTIA_PRICE_DAY_DATAS_QUERY, {
    variables: {
      duration,
    },
    ...
    const datas = data?.collectiveDayData?.nodes
      ?.map((bd: any) => {
        return {
          ...
          feeValueNative: new BigNumber(Number(bd?.totalDAFees)).toFormat(8),
        ...
        <div className="">
          <div className=" ">
            <div className=" w-full h-full text-sm leading-none flex items-center gap-2 ">
              ...
              <p className="opacity-80 ">TIA Price [Today]</p>
            </div>
            <div>
