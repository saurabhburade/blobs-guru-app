...
import { ETHEREUM_APP_DAY_DATAS_WITH_DURATION_QUERY } from "@/lib/apollo/queriesEthereum";
...
export default function AppExtCountDayChart({
  appId,
  duration,
}: {
  appId: string;
  ...
  const { data, loading } = useQuery(
    ETHEREUM_APP_DAY_DATAS_WITH_DURATION_QUERY,
    {
      variables: {
        duration: duration,
        appId: appId,
  ...
  const chartData = useMemo(() => {
    ...
    const datas = data?.appDayData?.nodes?.map((rawData: any) => {
      ...
      return {
        ...
        totalTxnCount: Number(rawData?.totalTxnCount),
  ...
  const cumulativeData = useMemo(() => {
    ...
    const totalExtCount = _.sumBy(data?.appDayData?.nodes, "totalTxnCount");
