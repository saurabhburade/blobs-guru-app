...
import {
  CELESTIA_APP_DA_FILTER_LIMIT_QUERY,
  CELESTIA_APP_DA_TRANSACTIONS_FILTER_LIMIT_QUERY,
  CELESTIA_SINGLE_APP_QUERY,
...
function SingleApp({ appId }: Props) {
  const { data, loading } = useQuery(CELESTIA_SINGLE_APP_QUERY, {
    variables: {
      appId: appId,
    },
...
function TxnRows({ account }: { account: string }) {
  ...
  const {
    ...
    loading,
    error,
  } = useQuery(CELESTIA_APP_DA_TRANSACTIONS_FILTER_LIMIT_QUERY, {
    variables: {
      namespaceID: account,
...
const TransactionRow = ({ txn }: any) => {
  const txFee = useMemo(() => {
    return new BigNumber(txn?.txFeeNative).toFormat(5);
  }, [txn?.txFeeNative]);
    ...
    <>
      <div className="hidden xl:grid xl:grid-cols-7 py-4 border-b border-base-200 text-sm items-center text-end">
        <div className="flex items-center gap-2 text-start">
          ...
          <div>
            <Link href={`/celestia/txn/${txn?.hash}`} className="text-primary">
              {" "}
        ...
        {parseCelestiaString(txn?.signerId) ? (
          <p>{formatAddress(parseCelestiaString(txn?.signerId))}</p>
        ) : (
          <p>-</p>
        )}
        {txn?.nEvents ? (
        ...
        <div>
          <p>{txn?.dataSubmission?.fees || 0} TIA </p>
        </div>
      </div>
      <div className="flex md:grid md:grid-cols-3 flex-wrap xl:hidden gap-2 lg:gap-0 justify-between first:border-t-0 border-t py-3 border-base-200 text-sm">
        <div className="flex items-center gap-2">
          ...
          <div>
            <Link href={`/celestia/txn/${txn?.id}`} className="text-primary">
              {" "}
        ...
        <div className="hidden  md:block xl:hidden text-end">
          <p className="lg:text-end ">
            From : {formatAddress(parseCelestiaString(txn?.signerId))}
          </p>
          <p className=" text-end">{txFee} TIA</p>
        </div>
