...
import { CELESTIA_ACCOUNTS_LIMIT_QUERY } from "@/lib/apollo/queriesCelestia";
import { formatAddress, formatBytes, parseCelestiaString } from "@/lib/utils";
...
function ChainAccounts({}: Props) {
  const [page, setPage] = useState(1);
  const { data: rawData, loading: statsLoading } = useQuery(
    CELESTIA_ACCOUNTS_LIMIT_QUERY,
    {
      ...
      <div className="hidden xl:grid xl:grid-cols-6 py-4 last:border-b-0 border-b border-base-200 text-sm items-center">
        <div className="flex items-center gap-2 col-span-2 ">
          ...
          <Link
            ...
            href={`/celestia/${parseCelestiaString(acc?.id)}`}
          >
