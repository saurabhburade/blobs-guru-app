...
export const shouldGetFees = (module: string): boolean => {
  const ignoreModules = ["timestamp", "authorship"];
...
export const handleVectorExecuteMessage = (message: Codec) => {
  ...
  } = message.toJSON() as any;
  if (
    newArg.message.fungibleToken &&
    !newArg.message.fungibleToken.amount.toString().startsWith("0x")
  ) {
    newArg.message.fungibleToken.amount = `0x${new BN(
      newArg.message.fungibleToken.amount.toString()
    ).toString("hex")}`;
...
export const handleVectorSendMessage = (message: Codec) => {
  let newArg: {
    fungibleToken?: { assetId: `0x${string}`; amount: number | `0x${string}` };
  } = message.toJSON() as any;
  if (
    newArg.fungibleToken &&
    !newArg.fungibleToken.amount.toString().startsWith("0x")
  ) {
    newArg.fungibleToken.amount = `0x${new BN(
      newArg.fungibleToken.amount.toString()
    ).toString("hex")}`;
    return JSON.stringify(newArg);
  } else {
    return message.toString();
