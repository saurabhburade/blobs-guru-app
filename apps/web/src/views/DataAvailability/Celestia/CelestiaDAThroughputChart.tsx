...
export default function CelestiaDAThroughputChart({ d }: { d: any }) {
  // const { data } = useQuery(BLOB_DAY_DATAS_QUERY);

  const chartData = useMemo(() => {
    const datas = d?.data?.series
      ?.map((bd: any) => {
        return {
          ...bd,
          valueF: new BigNumber(bd?.value).toFormat(4),
          timestampF: new Date(new Date(bd?.timestamp)),
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart width={500} height={100} data={chartData}>
          ...
          <Bar
            ...
            // shape={<TriangleBar />}
          />
          <XAxis dataKey="timestamp3" className="text-xs" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
...
const CustomTooltipRaw = ({ active, payload, label, rotation }: any) => {
  if (active && payload && payload.length) {
    return (
      ...
      >
        <div className="p-4 ">
          <p className=" ">Size: {`${payload[0]?.payload?.valueF} MiB/sec`}</p>
          <p className="  ">
            Timestamp: {`${payload[0]?.payload?.timestampF}`}
          </p>
        </div>
      </div>
    );
