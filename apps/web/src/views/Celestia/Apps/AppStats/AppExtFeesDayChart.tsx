...
export default function AppExtFeesDayChart({
  appId,
  duration,
}: {
  appId: string;
  ...
  const chartData = useMemo(() => {
    ...
    const datas = data?.appDayData?.nodes?.map((rawData: any) => {
      ...
      return {
        ...
        totalFeesNative: Number(rawData?.totalFeesNative),
        totalFeeNativeF: new BigNumber(rawData?.totalFeesNative).toFormat(4),
  ...
  const cumulativeData = useMemo(() => {
    ...
    const totalFees = _.sumBy(data?.appDayData?.nodes, "totalFeesNative");
    ...
    <div className="h-full w-full row-span-2 ">
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <Bar dataKey="totalFeesNative" fill="url(#colorUv)" radius={10}></Bar>
          <YAxis
    ...
    return (
      ...
      >
        ...
        <div className="px-4 space-y-3">
          <p className=" ">
            Ext Fee: {`${payload[0]?.payload?.totalFeeNativeF} TIA`}
          </p>
        </div>
      </div>
    );
