...
import { ETHEREUM_ACCOUNT_DAY_DATAS_WITH_DURATION_QUERY } from "@/lib/apollo/queriesEthereum";
...
export default function AccountExtFeesDayChart({
  account,
  duration,
}: {
  account: string;
  ...
  const { data, loading } = useQuery(
    ETHEREUM_ACCOUNT_DAY_DATAS_WITH_DURATION_QUERY,
    {
      variables: {
        address: account,
        duration: duration,
  ...
  const chartData = useMemo(() => {
    ...
    const datas = data?.accountDayData?.nodes?.map((rawData: any) => {
      ...
      return {
        ...
        totalFeeNative: Number(rawData?.totalFees) / 10 ** 18,
        totalFeeNativeF: new BigNumber(rawData?.totalFees)
          ?.div(10 ** 18)
          .toFormat(4),
  ...
  const cumulativeData = useMemo(() => {
    ...
    const totalFees = _.sumBy(data?.accountDayData?.nodes, "totalFees");
    ...
    return {
      ...
      totalFeesF: new BigNumber(totalFees)?.div(10 ** 18).toFormat(4),
      totalExtCount,
      totalFees,
    };
  }, [data?.accountDayData]);
  ...
  return (
    <div className="h-full w-full row-span-2 ">
      <div className="flex justify-between">
        ...
        <p className="text-xs">
          {cumulativeData?.totalFeesF} ETH [{duration} days]
        </p>
      </div>
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <Bar dataKey="totalFeeNative" fill="url(#colorUv)" radius={10}></Bar>
          <YAxis
    ...
    return (
      ...
      >
        ...
        <div className="px-4 space-y-3">
          <p className=" ">
            Txn Fee: {`${payload[0]?.payload?.totalFeeNativeF} ETH`}
          </p>
        </div>
      </div>
    );
