  ...
  const chartData = useMemo(() => {
    ...
    const datas = data?.appDayData?.nodes?.map((rawData: any) => {
      ...
      return {
        ...
        totalFeesNative: Number(rawData?.totalFeesNative) / 10 ** 18,
        totalFeeNativeF: new BigNumber(rawData?.totalFeesNative)
          ?.div(10 ** 18)
          .toFormat(4),
    ...
    return (
      ...
      >
        ...
        <div className="px-4 space-y-3">
          <p className=" ">
            Ext Fee: {`${payload[0]?.payload?.totalFeeNativeF} ETH`}
          </p>
        </div>
      </div>
    );
