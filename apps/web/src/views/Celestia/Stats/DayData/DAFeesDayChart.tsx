...
import {
  CELESTIA_DAY_DATAS_WITH_DURATION_QUERY,
  CELESTIA_DAY_DATAS_WITH_DURATION_WITH_ACCOUNTS_QUERY,
  CELESTIA_DAY_DATAS_WITH_DURATION_WITH_APPS_QUERY,
...
export default function DAFeesDayChart({ duration }: { duration: number }) {
  ...
  const chartDataFormated = useMemo(() => {
    ...
    for (let index = 0; index < accountDayDataParticipants?.length; index++) {
      ...
      for (let idx = 0; idx < accList?.length; idx++) {
        ...
        if (accDayData) {
          // @ts-ignore
          accDayDatasMap[`${accDayData?.appId}`] = accDayData?.totalDAFees;
          sum += Number(accDayData?.totalDAFees);
        }
      }
  ...
  const cumulativeData = useMemo(() => {
    ...
    const totalDAFees = _.sumBy(data?.collectiveDayData?.nodes, "totalDAFees");
    ...
    return {
      ...
      totalDataSubmissionCount,
      totalDAFeesF: new BigNumber(totalDAFees).toFormat(2),
      totalDAFees,
  ...
  return (
    <div className="h-full w-full row-span-2 ">
      <div className="flex justify-between">
        ...
        <p className="text-xs">
          {cumulativeData?.totalDAFeesF} TIA [{duration} days]
        </p>
      </div>
        ...
        >
          <div className="px-4 flex  gap-2 justify-between w-full ">
            ...
            <p className="h-full  flex justify-end w-full">
              {`${new BigNumber(total).toFormat(2)}`} TIA
            </p>
          </div>
              ...
              return (
                <div className="flex items-center gap-2" key={p?.dataKey}>
                  ...
                  <p className=" " key={p?.dataKey}>
                    ...
                    {`${new BigNumber(p?.value).toFormat(2)} TIA`}
                  </p>
                </div>
              );
            })}
