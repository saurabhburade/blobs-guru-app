...
import { CELESTIA_BLOCKS_LIMIT_QUERY } from "@/lib/apollo/queriesCelestia";
    ...
    <>
      <div className="hidden xl:grid xl:grid-cols-7 py-4 border-b border-base-200 text-sm items-center">
        <div className="flex items-center gap-2 text-start col-span-2">
          ...
          <div>
            <Link className="text-primary" href={`/celestia/blocks/${blk?.id}`}>
              {blockNumber}
            </Link>
            {/* <p>{blockNumber}</p> */}
        ...
        <div>
          <p>{Number(blk?.totalBlockFeeNatve)?.toFixed(4)} TIA</p>
        </div>
      </div>
