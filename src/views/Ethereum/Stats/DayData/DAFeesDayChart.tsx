...
import {
  ETHEREUM_DAY_DATAS_WITH_DURATION_QUERY,
  ETHEREUM_DAY_DATAS_WITH_DURATION_WITH_ACCOUNTS_QUERY,
...
export default function DAFeesDayChart({ duration }: { duration: number }) {
  ...
  const cumulativeData = useMemo(() => {
    ...
    return {
      ...
      totalDataSubmissionCount,
      totalDAFeesF: new BigNumber(totalDAFees)?.div(10 ** 18).toFormat(2),
      totalDAFees,
  ...
  return (
    <div className="h-full w-full row-span-2 ">
      <div className="flex justify-between">
        <p className="text-xs">DA Fees </p>
        <p className="text-xs">
          {cumulativeData?.totalDAFeesF} ETH [{duration} days]
        </p>
      </div>
              ...
              return (
                <div className="flex items-center gap-2" key={p?.dataKey}>
                  ...
                  <p className=" " key={p?.dataKey}>
                    ...
                    {`${new BigNumber(p?.value)?.div(10 ** 18).toFormat(2)} ETH`}
                  </p>
                </div>
              );
            })}
