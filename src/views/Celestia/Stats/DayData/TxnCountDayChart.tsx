...
export default function TxnCountDayChart({ duration }: { duration: number }) {
  ...
  const chartDataFormated = useMemo(() => {
    ...
    for (let index = 0; index < accountDayDataParticipants?.length; index++) {
      ...
      for (let idx = 0; idx < accList?.length; idx++) {
        ...
        if (accDayData) {
          // @ts-ignore
          accDayDatasMap[`${accDayData?.appId}`] = accDayData?.totalTxnCount;
          sum += Number(accDayData?.totalTxnCount);
        }
      }
      ...
      <>
        ...
        >
          <div className="px-4 flex  gap-2 justify-between w-full ">
            ...
            <p className="h-full  flex justify-end w-full">{`${total}`}</p>
          </div>
