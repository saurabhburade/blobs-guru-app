...
import {
  ...
  ETHEREUM_BLOCKS_WITH_LIMIT_QUERY,
...
const DAUtilisation: React.FC = () => {
  const { data: blockRes } = useQuery(ETHEREUM_BLOCKS_LIMIT_QUERY, {
    variables: {
      limit: LIMIT,
      skip: 0,
    },
  ...
  const memoOption = useMemo(() => {
    ...
    const utilisationPercent = new BigNumber(averageDataPerBlock)
      .div(Number(MAX_BLOBS_SIZE_TARGET_ETHEREUM))
      .multipliedBy(100)
      .toFormat(2);
      ...
      series: [
        {
          ...
          endAngle: -20,
          min: 0,
          max: Number(MAX_BLOBS_SIZE_TARGET_ETHEREUM) / 1024,
