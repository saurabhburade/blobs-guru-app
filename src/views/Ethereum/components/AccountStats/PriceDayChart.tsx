...
import { ETHEREUM_PRICE_DAY_DATAS_QUERY } from "@/lib/apollo/queriesEthereum";
...
export default function PriceDayChart({ duration }: { duration: number }) {
  const { data, loading } = useQuery(ETHEREUM_PRICE_DAY_DATAS_QUERY, {
    variables: {
      duration,
    },
    client: apolloClient,
    ...
    const datas = data?.collectiveDayData?.nodes
      ?.map((bd: any) => {
        return {
          ...
          feeValueNative: new BigNumber(Number(bd?.totalDAFees))
            ?.div(10 ** 18)
            .toFormat(8),
          ...
          avgEthPrice: 0,
          avgNativePrice: Number(bd?.avgNativePrice),
          totalBlobGasUSD: new BigNumber(Number(bd?.totalDAFeesUSD))
            ?.div(10 ** 18)
            .toNumber(),
          totalBlobGasUSDF: new BigNumber(Number(bd?.totalDAFeesUSD))
            ?.div(10 ** 18)
            .toFormat(2),
        };
      })
  ...
  const diffPercent = useMemo(() => {
    if (chartData) {
      ...
      const difference =
        todayData?.avgNativePrice - prevDayData?.avgNativePrice;
      const percentage = (difference / prevDayData?.avgNativePrice) * 100;
        ...
        <div className="">
          <div className=" ">
            <div className=" w-full h-full text-sm leading-none flex items-center gap-2 ">
              ...
              <p className="opacity-80 ">ETH Price [Today]</p>
            </div>
            <div>
              <p className=" leading-6 font-bold">
                <span className="font-normal">$</span>{" "}
                {chartData
                  ? new BigNumber(chartData?.at(-1)?.avgNativePrice)?.toFormat(
                      2
                    )
                  : "0"}{" "}
                <span
    ...
    return (
      ...
      >
        ...
        <div className="px-4 space-y-3">
          <p className=" ">
            ETH Price :{" "}
            {`$${new BigNumber(payload[0]?.payload?.avgNativePrice)?.toFormat(2)}`}{" "}
          </p>
        </div>
        <div className="px-4 space-y-3">
          <p className=" ">
