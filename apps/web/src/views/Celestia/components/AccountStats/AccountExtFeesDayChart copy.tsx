  ...
  const chartData = useMemo(() => {
    ...
    const datas = data?.accountDayData?.nodes?.map((rawData: any) => {
      ...
      return {
        ...
        totalFeeNative: Number(rawData?.totalFees),
        totalFeeNativeF: new BigNumber(rawData?.totalFees).toFormat(4),
  ...
  return (
    <div className="h-full w-full row-span-2 ">
      <div className="flex justify-between">
        ...
        <p className="text-xs">
          {cumulativeData?.totalFeesF} TIA [{duration} days]
        </p>
      </div>
    ...
    return (
      ...
      >
        ...
        <div className="px-4 space-y-3">
          <p className=" ">
            Txn Fee: {`${payload[0]?.payload?.totalFeeNativeF} TIA`}
          </p>
        </div>
      </div>
    );
