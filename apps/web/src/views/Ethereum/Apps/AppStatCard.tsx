...
type Props = {};
function AppStatCard({ acc, isLoading, className, chartData }: any) {
      ...
      <div className="flex gap-2 items-center border-b border-base-200/50  h-[4em] p-4">
        ...
        {!isLoading && (
          <>
            ...
            />
            <Link href={`/ethereum/apps/${acc?.id}`}>
              {accountDetails?.name ? (
...
const AccountExtChart = ({ account }: { account: string }) => {
  const { data } = useQuery(ETHEREUM_ACCOUNT_DAY_DATAS_WITH_DURATION_QUERY, {
    variables: {
      address: account,
      duration: 15,
    },
    ...
    return (
      ...
      >
        ...
        <div className="px-4 space-y-3">
          ...
          <p className=" ">
            Fee : {`${payload[0]?.payload?.totalFeeNative}`} ETH
          </p>
        </div>
      </div>
    );
