...
export default function AvailDAFeesDayChart({
  duration,
}: {
  duration: number;
}) {
  ...
  const cumulativeData = useMemo(() => {
    ...
    return {
      ...
      totalFeesAvail,
      totalFeesAvailF: new BigNumber(totalFeesAvail).toFormat(2),
      totalExtCount,
      totalByteSize,
  ...
  return (
    <div className="h-full w-full row-span-2 ">
      <div className="flex justify-between">
        ...
        <p className="text-xs">
          {cumulativeData?.totalFeesAvailF} AVAIL [{duration} days]
        </p>
      </div>
              ...
              return (
                <div className="flex items-center gap-2" key={p?.dataKey}>
                  ...
                  <p className=" " key={p?.dataKey}>
                    ...
                    {`${Number(p?.value)?.toFixed(2)} AVAIL`}
                  </p>
                </div>
              );
            })}
