...
import { CELESTIA_ACCOUNT_DAY_DATAS_WITH_DURATION_QUERY } from "@/lib/apollo/queriesCelestia";
...
export default function AccountExtCountDayChart({
  ...
  const { data, loading } = useQuery(
    CELESTIA_ACCOUNT_DAY_DATAS_WITH_DURATION_QUERY,
    {
      variables: {
        address: account,
        duration: duration,
  ...
  const chartData = useMemo(() => {
    ...
    const datas = data?.accountDayData?.nodes?.map((rawData: any) => {
      ...
      return {
        ...
        sizeValue: Number(rawData?.totalTxnCount),
        ...
        totalExtrinsicCount: Number(rawData?.totalTxnCount),
        totalFeeNative: new BigNumber(rawData?.totalFees).toFormat(4),
        totalExtrinsicCountF: new BigNumber(rawData?.totalTxnCount).toFormat(),
        timestamp: day,
  ...
  const cumulativeData = useMemo(() => {
    ...
    const totalExtCount = _.sumBy(data?.accountDayData?.nodes, "totalTxnCount");
  ...
  return (
    <div className="h-full w-full row-span-2 ">
      <div className="flex justify-between">
        ...
        <p className="text-xs">
          {cumulativeData?.totalExtCountF} [{duration} days]
        </p>
      </div>
    ...
    return (
      ...
      >
        ...
        <div className="px-4 space-y-3">
          <p className=" ">
            Txn Count: {`${payload[0]?.payload?.totalExtrinsicCountF}`}
          </p>
        </div>
      </div>
    );
