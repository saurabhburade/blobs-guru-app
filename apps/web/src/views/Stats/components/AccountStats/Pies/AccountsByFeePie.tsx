...
export default function AccountsByFeePie({ collectiveData }: any) {
  ...
  const chartData = useMemo(() => {
    ...
    const datas = processed?.map((bd: any, idx: number) => {
      const p = new BigNumber(Number(bd?.totalBlobGasEth))
        .div(Number(collectiveData?.totalBlobGasEth))
        .multipliedBy(100);
    ...
    const sorted = _.orderBy(
      allExceptLast,
      [(account) => parseInt(account.totalBlobGasEth)],
      ["desc"]
    );
