...
import {
  KB_PER_BLOB,
  MAX_BLOBS_TARGET,
  MAX_BLOBS_SIZE_TARGET_CELESTIA,
} from "@/configs/constants";
import BigNumber from "bignumber.js";
import MotionNumber from "motion-number";

import {
  CELESTIA_BLOCKS_LIMIT_QUERY,
  CELESTIA_BLOCKS_WITH_LIMIT_QUERY,
...
const DAUtilisation: React.FC = () => {
  const { data: blockRes } = useQuery(CELESTIA_BLOCKS_LIMIT_QUERY, {
    variables: {
      limit: LIMIT,
      skip: 0,
    },
  ...
  const memoOption = useMemo(() => {
    const totalSize = blockRes?.blockData?.nodes?.reduce(
      (acc: number, v: any) => {
        return acc + Number(v?.totalBlobSize);
      },
      0
    );
    const totalBlobTxCount = blockRes?.blockData?.nodes?.reduce(
      (acc: number, v: any) => {
        return acc + Number(v?.totalBlobTransactionCount);
      },
      0
    );

    const lastblockNumber = blockRes?.blockData?.nodes?.at(0)?.id;

    const averageBlobCount = new BigNumber(totalBlobTxCount)
      .div(blockRes?.blockData?.nodes?.length)
      .toNumber();
    const averageDataPerBlock = new BigNumber(totalSize)
      .div(blockRes?.blockData?.nodes?.length)
      .toNumber();

    const averageDataPerSub = new BigNumber(totalSize)
      .div(totalBlobTxCount)
      .toNumber();
    const utilisationPercent = new BigNumber(averageDataPerBlock)
      .div(Number(MAX_BLOBS_SIZE_TARGET_CELESTIA))
      .multipliedBy(100)
      .toFormat(2);
      ...
      series: [
        {
          ...
          endAngle: -20,
          min: 0,
          max: Number(MAX_BLOBS_SIZE_TARGET_CELESTIA) / 1024,
        ...
        <div className="grid lg:grid-cols-2 w-full h-full lg:h-[17em]">
          ...
          <div className="grid grid-cols-2  h-[15em] lg:h-full border-t lg:border-t-0 border-base-200">
            ...
            <div className="border-l border-base-200 flex flex-col items-center justify-center">
              <p className="opacity-70"> Avg. DA Tx/Block</p>
              <MotionNumber
                className="text-2xl font-bold gap-2"
                value={Number(memoOption?.averageBlobCount) || 0}
                last={() => <p className="text-2xl font-bold"></p>}
              />
            </div>
          </div>
        </div>
