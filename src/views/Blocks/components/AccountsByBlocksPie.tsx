...
import {
  ...
  TOP_BLOB_ACCOUNTS_BLOCK_PAGE_QUERY,
...
export default function AccountsByBlocksPie({ collectiveData }: any) {
  const { data } = useQuery(TOP_BLOB_ACCOUNTS_BLOCK_PAGE_QUERY);
  ...
  const chartData = useMemo(() => {
    ...
    const sorted = _.orderBy(
      allExceptLast,
      [(account) => parseInt(account.totalBlobBlocks)],
      ["desc"]
    );
        ...
        <div className="w-[20em] h-[20em] overflow-hidden">
          ...
          <PieChart width={400} height={400} className=" p-0">
            ...
            <Legend
              verticalAlign="top"
              content={() => (
                <span className="text-xs px-4">Blob blocks count</span>
              )}
            />
            ...
            >
              ...
              <p className="text-xs text-end flex gap-2">
                <span className="opacity-25">
                  {" "}
                  {numberFormater.format(entry?.totalBlobBlocks)}
                </span>{" "}
    ...
    return (
      ...
      >
        <div className="p-4 ">
          ...
          <p className=" ">
            Blobs:{" "}
            {`${new BigNumber(payload[0]?.payload?.totalBlobTransactionCount).toFormat()}`}
          </p>
        </div>
      </div>
    );
